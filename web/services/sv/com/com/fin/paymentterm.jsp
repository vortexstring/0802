...
<%!
    String user_id, payterm_name, comments, createby_id, writeby_id, writedt, company_id;
...
// DETERMINE THE ACTION TYPE AND CALL THE APPROPRIATE METHOD THAT RETURNS A RESPONSE
        Paymentterm crudPayterm = new Paymentterm();
        if (actiontype.equals("0")) {
            if (id.equals("-1")) {
                myfeedback = crudPayterm.cud("c", jsonData);
                out.print(myfeedback);
            } else {
                myfeedback = crudPayterm.cud("u", jsonData);
                out.print(myfeedback);
            }
        } else if (actiontype.equals("-5")) {
            myfeedback = crudPayterm.cud("d", jsonData);
            out.print(myfeedback);
        }
    } catch (Exception e) {
