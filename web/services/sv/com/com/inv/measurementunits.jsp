...
<%@ page import="model.*,crud.inv.*" %>




<%!
    String uom_name, uom_category_id;
...
// DETERMINE THE ACTION TYPE AND CALL THE APPROPRIATE METHOD THAT RETURNS A RESPONSE
        CrmUomCrud crudCrmUom = new CrmUomCrud();
        if (actiontype.equals("0")) {
            if (id.equals("-1")) {
                myfeedback = new Gson().toJson(crudCrmUom.cud("c", jsonData));
                out.print(myfeedback);
            } else {
                myfeedback = new Gson().toJson(crudCrmUom.cud("u", jsonData));
                out.print(myfeedback);
            }
        } else if (actiontype.equals("-5")) {
            myfeedback = new Gson().toJson(crudCrmUom.cud("d", jsonData));
            out.print(myfeedback);
        }
    } catch (Exception e) {
